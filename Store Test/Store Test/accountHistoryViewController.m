...
- (void)viewDidLoad
{
    ...
    accountHistoryTableView.dataSource = self;
    accountHistoryTableView.delegate = self;
    [super viewDidLoad];
	// Do any additional setup after loading the view.
    
    accountHistoryTableView.scrollEnabled = YES;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if(indexPath.section == 0){
        ...
        if (!loadedStudent) {
            loadedStudent = [[studentAccounts alloc] initWithInformation:0 :0 :@"Student Information"];
            
        }
        
        [accountHistoryCell updateInformation:[UIImage imageNamed:[NSString stringWithFormat:@"%@.png", loadedStudent.name]] :loadedStudent.name :loadedStudent.totalSpent :loadedStudent.totalDeposited :loadedStudent.currentBalance: YES];
...
- (void) studentWasSelected: (studentAccounts*) studentsAccount{
    transactionCosts = [[NSArray alloc] initWithArray:[studentsAccount pricesInStoreTransactions]];
    transactions = [[NSArray alloc] initWithArray:[studentsAccount itemsInStoreTransactions]];
    
    numTransactions = [transactions count];
    
    [accountHistoryTableView reloadData];
